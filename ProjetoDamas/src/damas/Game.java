...
public class Game {
    ...
    public void start() {
        ...
        while (jogar) {
            ...
            int[] origemCoords = getCoordinates("Digite a linha e coluna da peça que deseja mover (ex: 2 3): ");
            if (origemCoords == null) {
                jogar = false;
                continue;
            }

            int[] destinoCoords = getCoordinates("Digite a linha e coluna da casa destino (ex: 3 4): ");
    ...
    private void movePiece(int linhaOrigem, int colunaOrigem, int linhaDestino, int colunaDestino) {
        ...
        if (isValidMove(linhaOrigem, colunaOrigem, linhaDestino, colunaDestino)) {
	        ...
	        if ((peca == 'w' || peca == 'b' || peca == 'W' || peca == 'B') && (casaDestino == 'o')){
	            ...
	            if ((peca == 'w' || peca == 'W') && currentPlayer.getColor() == Color.white ||
	                (peca == 'b' || peca == 'B') && currentPlayer.getColor() == Color.black) {
    ...
    private boolean isValidMove(int linhaOrigem, int colunaOrigem, int linhaDestino, int colunaDestino) {
        ...
        if ((linhaDestino - linhaOrigem) == 0 || (colunaDestino - colunaOrigem) == 0) {
        	return false;
        } else if (Math.abs(linhaDestino - linhaOrigem) == 1 && Math.abs(colunaDestino - colunaOrigem) == 1) { //Movimento não é captura
