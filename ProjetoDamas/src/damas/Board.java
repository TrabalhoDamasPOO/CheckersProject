...
public class Board {
	    ...
	    public static int getDist(int origem, int destino) {
	    ...
	    public boolean isKing(int lin, int col) {
	    	...
	    	char peca = tabuleiro[col + (lin*DEFAULT_SIZE)];
	    	if ((peca == 'W') || (peca == 'B')) {
	    		return true;
	    	} else {
	    		return false;
	    ...
	    public int scanCapturasDistantes(int lin, int col, boolean whitePlayer) {
	    	...
	    	int indiceAux; //Índice auxiliar para verificações
	    	int contador = 0;
	    	...
	    	for (int i = 1; i < 6; i++) {
	    		...
	    		//Verificar se existe peça atrás sem ser a peça de origem
	    		if (i > 1) {
	    			indiceAux = (col+((i-1)*dir[1])) + ((lin+((i-1)*dir[0]))*DEFAULT_SIZE);
	    			if ((indiceAux >= 0) && (indiceAux < DEFAULT_SIZE*DEFAULT_SIZE)) { //Evitar trabalhar com valores fora dos limites do tabuleiro
		    			if (tabuleiro[indiceAux] == 'o') {
	    		...
	    		indice1 = (col-(i*dir[1])) + ((lin-(i*dir[0]))*DEFAULT_SIZE);
	    		indice2 = (col-((i+1)*dir[1])) + ((lin-((i+1)*dir[0]))*DEFAULT_SIZE);
	    		//Mesma verificação, no entanto na direção espelhada
	    		if (i > 1) {
	    			indiceAux = (col-((i-1)*dir[1])) + ((lin-((i-1)*dir[0]))*DEFAULT_SIZE);
	    			if ((indiceAux >= 0) && (indiceAux < DEFAULT_SIZE*DEFAULT_SIZE)) {
		    			if (tabuleiro[indiceAux] == 'o') {
		    				contador += validacaoCaptura(indice1, indice2, whitePlayer);
		    			}
	    			}
